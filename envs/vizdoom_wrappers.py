...
def wrap_lightzero(config: EasyDict) -> gym.Env:
    ...
    if getattr(config, 'oc_model', False):
        from ocrs.slotcontrast import load_from_checkpoint as load_slotcontrast_from_ckpt
        device = getattr(config, "oc_device", "cuda")
        ocr_config_path = _resolve_path(config.ocr_config_path)
        checkpoint_path = _resolve_path(config.checkpoint_path)
        ...
        slot_extractor = SlotExtractor(model=slotcontrast, device=device, name_model=config.oc_model_type)
...
class ExtractObservation(gym.ObservationWrapper):
    def __init__(self, env: gym.Env, key: str = 'screen'):
        super().__init__(env)
        assert isinstance(env.observation_space, (gymnasium.spaces.Dict, gym.spaces.Dict)), type(env.observation_space)
...
class GymnasiumToGymWrapper(gym.Wrapper):
    ...
    def __init__(self, env):
        ...
        super().__init__(env)
        self._seed = None
        self.observation_space = self._to_gym_space(env.observation_space)
        self.action_space = self._to_gym_space(env.action_space)
    ...
    def _to_gym_space(space):
        ...
        if isinstance(space, gymnasium.spaces.Dict):
            return gym.spaces.Dict({k: GymnasiumToGymWrapper._to_gym_space(v) for k, v in space.spaces.items()})
