...
def wrap_lightzero(config: EasyDict, seed: int = 0) -> gym.Env:
    """
    Overview:
    ...
    """
    env = RobosuiteEnv(task='Lift', horizon=config.max_episode_steps, seed=seed)
    ...
    if config.oc_model:
        ...
        if oc_model_type == 'SAVi':
            ...
            state_dict = torch.load(_resolve_path(config.checkpoint_path))['state_dict']
...
class SlotExtractorWrapper(gym.Wrapper):
    ...
    def _get_slots(self, frame, prev_slots=None):
        if self.slot_extractor.name_model == 'DINOSAUR':
            # DINOSAUR rollout for Robosuite should stay stateless.
            return self.slot_extractor(frame, prev_slots=None)
        if self.slot_extractor.name_model == 'SLATE' and prev_slots is None:
